...
const Event = () => {
  ...
  const eventUrl = `http://localhost:5173/event/${id}`;
  ...
  const handleDeleteItem = (idItem) => {
    api
      .delete(`/item/${idItem}`)
      .then((res) =>
        setItems((prevItems) => prevItems.filter((item) => item.id !== idItem))
      )
      .catch((err) => console.log('ERRREUR: ', err));
  };
  ...
  const handleRequestPasswordSubmit = (e) => {
    ...
    api
      ...
      .catch((err) => {
        ...
        if (err.code === 'ERR_BAD_REQUEST') {
          setIsAllowedToSeeEvent(false);
          setRequestAccessPasswordError("Le mot de passe n'est pas correct.");
        }
      });
  };
      ...
      <div className="event">
        ...
        <div className="event__container">
          ...
          <div className="event__date">
            <p className="event__container-date">
        ...
        <div className="event__info">
          ...
          <div className="event__participant">
            <PlaceIcon />
            <p>
              {event.address}, {event.postal}{' '}
              <span className="event__address">{event.city}</span>{' '}
            </p>
          </div>
          {
            <div className="event__participant">
              ...
              <p>
                {users.length}{' '}
                {users.length < 2 ? 'participant' : 'participants'}
              </p>
            </div>
          }
        ...
        <div className="event__icons">
          ...
          <div className="icon">
            <div className="icon__notification">1</div>
            <LunchDiningIcon fontSize="large" onClick={handleClickModal} />
          </div>
          <div className="icon">
